    ...
    <body>
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			<?php
                ...
                $paymentDue = 0; 
                $selQ = "SELECT * FROM tbl_studentreg";
                $resQ = $conn->query($selQ);
                while($dataQ = $resQ->fetch_assoc())
                {
                    $i++;
                    ?>
                        <tr>
                            <td><?php echo $i; ?></td>
                            <td><?php echo $dataQ["stu_name"]; ?></td>
                            <?php
                                foreach ($months as $index => $month) {
                                    
                                    $currentYear = date('Y');
                                    $s = "SELECT * FROM tbl_payment WHERE stureg_id=" . $dataQ["stureg_id"] . " AND pay_month='" . $month . "' AND YEAR(pay_datetime)='" . $currentYear . "' ";
                                    $res1 = $conn->query($s);
                                    if ($res1->num_rows > 0) {
                                        echo "<td style='color:green'>Paid</td>";
                                    } else {
                                        echo "<td style='color:red'>Unpaid</td>";
                                        $sel = "select * from tbl_stdstp s inner join tbl_stop p on p.stop_id=s.stop_id inner join tbl_route r on s.stureg_id=" . $dataQ["stureg_id"];
                                        $res = $conn->query($sel);
                                        if ($row = $res->fetch_assoc()) {
                                    if($row['stp_no']<=5)
                                        $paymentDue += $row['route_rate'];
                                    else 
                                    $paymentDue += $row['route_rate']+100;
                                    }
                                    }
                                   
                                }
                            ?>
                            <td><?php echo $paymentDue ?></td>
                        </tr>
                    <?php
                }
